...
fn api() -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    url_by_app_config().or(oauth_authorize())
}
...
async fn url_by_app_config_body(app_config: AppConfig) -> Result<impl warp::Reply, Infallible> {
...
async fn url_by_app_config_inner(app_config: AppConfig) -> anyhow::Result<Response<Body>> {
    ...
    let state =
        toml::to_string(&app_config).map_err(|e| anyhow::anyhow!("序列化配置时出错: {}", e))?;
    let state = crate::custom_crypto::encrypt_buff_to_base64(state.as_bytes(), PASSWORD.as_bytes())
...
async fn oauth_authorize_body_inner(
    ...
    let state = crate::custom_crypto::decrypt_base64(state, PASSWORD.as_bytes())
